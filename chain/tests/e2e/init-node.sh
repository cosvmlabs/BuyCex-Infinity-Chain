...
node_address=$(cvmd keys list | grep  "address: " | cut -c12-)
...
cvmd add-genesis-account $KEY 100000000000000000000000000aevmos --keyring-backend $KEYRING
...
cvmd gentx $KEY 1000000000000000000000aevmos --keyring-backend $KEYRING --chain-id "$CHAINID"
## In case you want to create multiple validators at genesis
## 1. Back to `cvmd keys add` step, init more keys
## 2. Back to `cvmd add-genesis-account` step, add balance for those
## 3. Clone this ~/.cvmd home directory into some others, let's say `~/.clonedEvmosd`
## 4. Run `gentx` in each of those folders
## 5. Copy the `gentx-*` folders under `~/.clonedEvmosd/config/gentx/` folders into the original `~/.cvmd/config/gentx`
...
cvmd start "$TRACE" --log_level $LOGLEVEL --minimum-gas-prices=0.0001aevmos --json-rpc.api eth,txpool,personal,net,debug,web3
